...
- Admin pages excluded from analytics: AnalyticsBeacon skips `/admin*` paths (client) AND /api/analytics drops them (server); historical admin PAGE_VIEW/PAGE_LEAVE rows (6,702) deleted 2026-07-31 — daily view counts are public traffic only
- Stale-deploy chunk errors self-heal: ClientErrorReporter reloads once on ChunkLoadError/Loading-chunk-failed (60s sessionStorage guard against loops); still reported as WARN first
- Worker claim endpoint (`/api/worker/video-processing/claim`) wraps its queries in `withDbRetry` — transient P2024/P1001 blips (deploy churn, heavy admin ops) ride a backoff instead of paging the monitor; the 2026-08-01 00:26–00:30 UTC P2024 burst was contention from the one-off analytics cleanup delete, not a config regression (printed limit:10/timeout:15 = pool params active)
- Admin table buttons are `tbl-btn`/`tbl-btns` (renamed from `action-btn` 2026-08-01): the old name collided with the public engagement pills in `components/action-buttons.css` — public CSS persists across client-side navigation into /admin and blew up the admin buttons. Never reuse public class names in admin. Works table is 12 cols (Pos. col 11 @132px, Actions col 12 @128px); indicator headers are lucide icons w/ tooltips
...
## Legacy locale 404s → 301 redirects — 2026-07-31
