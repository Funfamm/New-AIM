...
import { updateSubtitleById } from "@/lib/subtitles/subtitle-repo";
...
type ProgressBody = {
  ...
  // On READY: full translations + vtt keys
...
export async function POST(req: NextRequest) {
  ...
  const { jobId, status, progress, error, apiKeyId, translations, vttKeys } = body;
  ...
  if (status === "READY") {
    ...
    if (translations && vttKeys) {
      await updateSubtitleById(job.subtitleId, { translationsJson: translations, vttKeysJson: vttKeys });
    }

    // Track success against the DB key (no-op for env fallback where apiKeyId is null)
    if (apiKeyId) {
      await markKeySuccess(apiKeyId);
    }
    ...
    if (apiKeyId) {
      await markKeyFailure(apiKeyId, errorMsg);
    }

    // Auto-requeue if another healthy key is available — the next claim will pick a different key
