// Admin Settings — tabbed layout to prevent long scroll
// Server-rendered. Tab state is URL-based (?tab=...). Client components for save buttons + security forms.
...
export default async function SettingsPage({
  ...
  const tab = (TABS.some((t) => t.key === sp.tab) ? sp.tab : "email") as TabKey;
      ...
      <nav className="stg-tabs">
        {TABS.map(({ key, label }) => (
          <a
            ...
            className={`stg-tab${tab === key ? " stg-tab--active" : ""}`}
          >
            {label}
          </a>
        ))}
